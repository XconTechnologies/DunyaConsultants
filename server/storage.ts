...
export interface IStorage {
  ...
  updateQrCode(id: number, data: Partial<Pick<QrCode, 'title' | 'link'>>): Promise<QrCode>;
...
export class DatabaseStorage implements IStorage {
  ...
  async updateQrCode(id: number, data: Partial<Pick<QrCode, 'title' | 'link'>>): Promise<QrCode> {
  ...
  // Optimized version for listing - excludes content field to reduce payload size
  async getBlogPostsForList(published?: boolean): Promise<any[]> {
    if (published !== undefined) {
      const posts = await db.select({
        id: blogPosts.id,
        title: blogPosts.title,
